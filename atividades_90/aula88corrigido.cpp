...
#include <iostream>
#include <list>
#include <algorithm> // funcaoo find
...
{
	// adiciona vertice v2 e lista de vertices adjacentes de v1
	adj[v1].push_back(v2);
}
...
void Grafo::dfs(int v)
{
	...
	while(true)
	{
		...
		{
			// se todos os vizinhos estao visitados ou nao existem vizinhos
			// remove da pilha
			pilha.pop();
			// se a pilha ficar vazia, entao terminou a busca
			if(pilha.empty())
				break;
			// se chegou aqui, e porque pode pegar elemento do topo
			v = pilha.top();
		}
	}
}
